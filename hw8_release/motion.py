...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        b = -(img1[y-w:y+w+1, x-w:x+w+1] * It[y-w:y+w+1, x-w:x+w+1]).flatten().reshape(window_size**2, 1)
        A = np.array([(img1[y-w:y+w+1, x-w:x+w+1] * Ix[y-w:y+w+1, x-w:x+w+1]).flatten(), (img1[y-w:y+w+1, x-w:x+w+1] * Iy[y-w:y+w+1, x-w:x+w+1]).flatten()]).T
        
        A_inv = np.linalg.inv(np.dot(A.T,A))
        v = np.dot(A_inv,np.dot(A.T,b))
...
def iterative_lucas_kanade(img1, img2, keypoints,
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        Ix_sum = (Ix[y1-w:y1+w+1, x1-w:x1+w+1]**2).sum()
        Iy_sum = (Iy[y1-w:y1+w+1, x1-w:x1+w+1]**2).sum()
        Ixy_sum = (Ix[y1-w:y1+w+1, x1-w:x1+w+1]*Iy[y1-w:y1+w+1, x1-w:x1+w+1]).sum()
        G = np.array( [[Ix_sum, Ixy_sum] , [Ixy_sum, Iy_sum]] )
        ...
        for k in range(num_iters):
            ...
            delk = img1[y1-w:y1+w+1, x1-w:x1+w+1] - img2[y2-w:y2+w+1, x2-w:x2+w+1]
            bk = np.array([(delk*Ix[y1-w:y1+w+1, x1-w:x1+w+1]).sum(),  (delk*Iy[y1-w:y1+w+1, x1-w:x1+w+1]).sum() ])
            vk = np.dot(G_inv, bk)
...
def compute_error(patch1, patch2):
    ...
    error = 0
    error = (((patch1-np.mean(patch1))/np.std(patch1) - (patch2-np.mean(patch2))/np.std(patch2))**2).mean()
    return error
...
def IoU(bbox1, bbox2):
    ...
    if x1+w1 < x2 or x2+w2 < x1:
        return 0
    if y1+h1 < y2 or y2+h2 < h2:
        return 0

    x = np.array([x1, x1+w1, x2, x2+w2])
    y = np.array([y1, y1+h1, y2, y2+h2])
    x_sort = np.sort(x)
    y_sort = np.sort(y)
    
    intersect = (x_sort[2]-x_sort[1])*(y_sort[2]-y_sort[1])
    
    score = intersect/((w1*h1) + (w2*h2) - intersect)
