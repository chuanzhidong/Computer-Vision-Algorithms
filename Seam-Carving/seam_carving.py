...
def compute_cost(image, energy, axis=1):
    ...
    for r in range(1,H):
        prev_energy = cost[r-1]
        center = prev_energy
        left = np.append([float("inf")], prev_energy[:-1])
        right = np.append(prev_energy[1:],[float("inf")])
        combo = np.stack((center, left, right))
               
        min_energy = np.amin(combo, axis = 0)
       
        cost[r] = energy[r] + min_energy
        value = np.argmin(combo, axis = 0)
        
        np.place(value,value==1, -1)
        np.place(value,value==2, 1)
        paths[r] = value
...
def remove_seam(image, seam):
    ...
    for row in range(H):
        out[row] = np.delete(image[row], seam[row], axis=0)
...
def duplicate_seam(image, seam):
    ...
    H, W, C = image.shape
    out = np.zeros((H, W + 1, C))
    #np.array([np.insert(image[row],seam[row], image[row, seam[row]], axis=0) for row in range(H)])
    for i in range(H):
        out[i] = np.array([np.insert(image[i,:],seam[i], image[i][seam[i]], axis=0)])
        #i is row
        #seam[i] is the column
    
    return out

def duplicate_seam_2d(image, seam):
    """Duplicates pixels of the seam, making the pixels on the seam path "twice larger".
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    for value in range(size-W):
        temp = np.where(seams == value+1)[1]
        out = duplicate_seam(out, temp)
        seams = duplicate_seam_2d(seams,temp)
...
def compute_forward_cost(image, energy):
    ...
    for r in range(1,H):
        ...
        left = np.zeros(W)
        left[0] = float("inf")
        left[1:] = prev_energy[:-1]
        right = np.zeros(W)
        right[:-1] = prev_energy[1:]
        right[-1] = float("inf")
       ...
       # #I_i_j_plus_1 = np.append(curr_row_image[1:],[0])
        #I_i_j_minus_1 = np.append([0], curr_row_image[:-1])
        #I_i_minus_1_j = prev_row_image
        ...
        #CV = np.zeros(W)
        #CV = np.absolute(I_i_j_plus_1-I_i_j_minus_1)
        
        
        CV = np.zeros(W)
        CV[1:-1] = np.absolute(j_plus_1[1:] - j_minus_1[:-1])
        
        CL = np.copy(CV)
        CL[1:] += np.absolute(i_minus_1[1:] - j_minus_1)
        CR = np.copy(CV)
        CR[:-1] += np.absolute(i_minus_1[:-1] - j_plus_1)                      
        ...
        #combo = np.stack((add_C, add_L, add_R))
        combo = np.array([left+CL, center+CV, right+CR])
        ...
        #np.place(value,value==1, -1)
        #np.place(value,value==2, 1)
        paths[r] = np.argmin(combo, axis = 0) -1
