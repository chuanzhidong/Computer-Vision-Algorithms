...
def compute_distances(X1, X2):
    ...
    #P = np.add.outer(np.sum(np.square(X1), axis=1), np.sum(np.square(X2), axis=1))
    #dists = np.sqrt(P - 2 * np.dot(X1, X2.T))
    
    dists = np.sqrt(-2 * np.dot(X1, X2.T) + np.sum(X2**2, axis=1) + np.sum(X1**2, axis=1)[:, np.newaxis])
...
def split_folds(X_train, y_train, num_folds):
    ...
    X_vals = np.array(np.array_split(X_train, num_folds))
    y_vals = np.array(np.array_split(y_train, num_folds))
    
    j=0
    for i in range(num_folds):
        X_trains[i] = np.concatenate((X_train[:j], X_train[j+validation_size:]))
        y_trains[i] = np.concatenate((y_train[:j], y_train[j+validation_size:]))
        j += validation_size
